  ...
  const data = allDates.map((date) => {
    return {
      ...
      totalSales: bookings
        ...
        .reduce((cur, arr) => cur + arr.totalPrice, 0),

      extrasSales: bookings
        .filter((booking) => isSameDay(date, new Date(booking.created_at)))
        .reduce((cur, arr) => cur + arr.extrasPrice, 0),
    };
  });
  ...
  return (
    <StyledSalesChart>
      ...
      <ResponsiveContainer height={300} width={'100%'}>
        <AreaChart data={data} margin={{ bottom: 16 }}>
          <XAxis
            dataKey={'label'}
          ...
          <Tooltip
            ...
            formatter={(value) => new Intl.NumberFormat('id').format(value)}
          />
